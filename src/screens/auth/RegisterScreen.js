 ...
 *
 * Updated: Full-screen loading overlay with dual-ring spinner + cycling messages.
 */
...
const BounceDot = ({ delay }) => {
  ...
  return (
    <Animated.View
      style={[overlayStyles.bounceDot, { transform: [{ translateY: bounce }] }]}
    />
  );
};
...
const LoadingOverlay = ({ visible, message }) => {
  ...
  // Fade-in message when it changes
  useEffect(() => {
    msgFade.setValue(0);
    Animated.timing(msgFade, { toValue: 1, duration: 350, useNativeDriver: true }).start();
  }, [message]);

  const outerSpin = spinAnim.interpolate({ inputRange: [0, 1], outputRange: ["0deg",   "360deg"] });
  const innerSpin = spinAnim.interpolate({ inputRange: [0, 1], outputRange: ["360deg", "0deg"]   });
...
const BackgroundOrbs = () => (
  <View style={StyleSheet.absoluteFill} pointerEvents="none">
    ...
    <View style={[styles.orb, { width: 200, height: 200, borderRadius: 100, bottom: 80, left: -50, backgroundColor: "rgba(99,179,255,0.09)" }]} />
    <View style={[styles.orb, { width: 140, height: 140, borderRadius: 70, top: "45%", left: width / 2 - 70, backgroundColor: "rgba(139,92,246,0.07)" }]} />
  </View>
);
...
const StepDots = ({ total = 2, current = 0 }) => (
  <View style={styles.dotsRow}>
    ...
    <View style={[styles.dotTrack, { width: (total - 1) * 36 }]}>
      <View style={[styles.dotFill, { width: `${(current / (total - 1)) * 100}%` }]} />
    </View>
  </View>
);
...
const PremiumField = memo(({
  label, fieldKey, form, update, placeholder,
  keyboardType, maxLength, secureTextEntry,
  autoCapitalize, icon, optional = false,
}) => {
  ...
  return (
    <View style={fieldStyles.group}>
      ...
      <Animated.View style={[fieldStyles.wrapper, { borderColor, backgroundColor: bgColor }]}>
        {icon && (
...
const fieldStyles = StyleSheet.create({
  ...
  label:         { fontSize: 11, fontWeight: "600", color: "rgba(255,255,255,0.4)", letterSpacing: 1, textTransform: "uppercase" },
  optionalBadge: { fontSize: 9, fontWeight: "700", color: "rgba(139,92,246,0.7)", backgroundColor: "rgba(139,92,246,0.12)", borderRadius: 6, paddingHorizontal: 6, paddingVertical: 2, letterSpacing: 0.5, textTransform: "uppercase" },
...
const RegisterScreen = ({ navigation }) => {
  const [form, setForm] = useState({
    name: "", phone: "", email: "",
    password: "", businessName: "", gstin: "", referCode: "",
  });
  ...
  useEffect(() => {
    Animated.stagger(90, [
      Animated.spring(backAnim,  { toValue: 1, tension: 70, friction: 10, useNativeDriver: true }),
      Animated.spring(titleAnim, { toValue: 1, tension: 65, friction: 10, useNativeDriver: true }),
      Animated.spring(card1Anim, { toValue: 1, tension: 55, friction: 10, useNativeDriver: true }),
      Animated.spring(card2Anim, { toValue: 1, tension: 55, friction: 10, useNativeDriver: true }),
      Animated.spring(btnAnim,   { toValue: 1, tension: 55, friction: 10, useNativeDriver: true }),
    ]).start();
  }, []);
          ...
          >
            ...
            <Animated.View style={[styles.cardShadow, slideUp(card2Anim)]}>
              <View style={styles.card}>
                ...
                <View style={styles.cardHeader}>
                  <View style={[styles.cardHeaderIcon, { backgroundColor: "rgba(5,150,105,0.15)" }]}>
                ...
                <View style={{ marginTop: 8 }}>
                  ...
                  />
                  <View style={styles.gstinHint}>
                    <Ionicons name="people-outline" size={13} color="rgba(255,255,255,0.25)" />
                    <Text style={styles.gstinHintText}>
            ...
            <Animated.View style={[{ transform: [{ scale: btnScale }] }, slideUp(btnAnim)]}>
              <TouchableOpacity
            ...
            <Animated.View style={[styles.signinRow, slideUp(btnAnim)]}>
              ...
              <Ionicons name="chevron-forward" size={13} color="#8B5CF6" style={{ marginLeft: 2 }} />
            </Animated.View>

            {/* ── Terms note ──────────────────────────────────────────────── */}
            <Animated.View style={[styles.termsRow, slideUp(btnAnim)]}>
              <Ionicons name="shield-checkmark-outline" size={12} color="rgba(255,255,255,0.18)" />
...
const styles = StyleSheet.create({
  ...
  backBtn:   { width: 38, height: 38, borderRadius: 13, backgroundColor: "rgba(255,255,255,0.07)", borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", alignItems: "center", justifyContent: "center" },
  backLabel: { fontSize: 14, color: "rgba(255,255,255,0.4)", fontWeight: "500" },

  // Title
  titleBlock:     { marginBottom: 24 },
  titleBadge:     { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "rgba(108,99,255,0.12)", borderRadius: 20, paddingHorizontal: 10, paddingVertical: 5, alignSelf: "flex-start", marginBottom: 12, borderWidth: 1, borderColor: "rgba(108,99,255,0.2)" },
  titleBadgeDot:  { width: 5, height: 5, borderRadius: 2.5, backgroundColor: "#8B5CF6" },
  titleBadgeText: { fontSize: 11, fontWeight: "700", color: "#A78BFA", letterSpacing: 0.5 },
  headline:       { fontSize: 30, fontWeight: "900", color: "#FFFFFF", letterSpacing: -0.8, lineHeight: 36, marginBottom: 10 },
  subline:        { fontSize: 14, color: "rgba(255,255,255,0.38)", lineHeight: 20 },

  // Cards
  cardShadow:      { shadowColor: "#6C63FF", shadowOffset: { width: 0, height: 10 }, shadowOpacity: 0.18, shadowRadius: 24, elevation: 14, borderRadius: 24, marginBottom: 14 },
  card:            { backgroundColor: "rgba(255,255,255,0.05)", borderRadius: 24, borderWidth: 1, borderColor: "rgba(255,255,255,0.08)", padding: 20, paddingTop: 6, overflow: "hidden" },
  cardTopBar:      { height: 2, borderRadius: 1, marginBottom: 20 },
  cardHeader:      { flexDirection: "row", alignItems: "center", gap: 8, marginBottom: 18 },
  cardHeaderIcon:  { width: 26, height: 26, borderRadius: 8, backgroundColor: "rgba(108,99,255,0.15)", alignItems: "center", justifyContent: "center" },
  cardHeaderLabel: { fontSize: 13, fontWeight: "700", color: "rgba(255,255,255,0.55)", letterSpacing: 0.3, flex: 1 },
  cardOptBadge:    { backgroundColor: "rgba(16,185,129,0.12)", borderRadius: 8, paddingHorizontal: 8, paddingVertical: 3, borderWidth: 1, borderColor: "rgba(16,185,129,0.2)" },
  cardOptText:     { fontSize: 9, fontWeight: "700", color: "#10B981", letterSpacing: 0.5, textTransform: "uppercase" },

  // GSTIN hint
  gstinHint:     { flexDirection: "row", alignItems: "center", gap: 6, backgroundColor: "rgba(255,255,255,0.03)", borderRadius: 10, padding: 10, marginTop: -4 },
  gstinHintText: { fontSize: 11, color: "rgba(255,255,255,0.25)", lineHeight: 16, flex: 1 },

  // Create button
  createBtn:      { height: 56, borderRadius: 16, flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 10, overflow: "hidden", shadowColor: "#6C63FF", shadowOffset: { width: 0, height: 6 }, shadowOpacity: 0.45, shadowRadius: 16, elevation: 10 },
  createBtnGlow:  { position: "absolute", top: -18, width: 120, height: 56, borderRadius: 28, backgroundColor: "rgba(255,255,255,0.12)" },
  createBtnText:  { fontSize: 16, fontWeight: "800", color: "#fff", letterSpacing: 0.3 },
  createBtnArrow: { width: 28, height: 28, borderRadius: 14, backgroundColor: "#fff", alignItems: "center", justifyContent: "center" },

  // Sign in link
  signinRow:    { flexDirection: "row", alignItems: "center", justifyContent: "center", marginTop: 18 },
  signinText:   { fontSize: 14, color: "rgba(255,255,255,0.4)" },
  signinAccent: { fontSize: 14, fontWeight: "700", color: "#8B5CF6" },

  // Terms
  termsRow:  { flexDirection: "row", alignItems: "center", justifyContent: "center", gap: 6, marginTop: 14 },
  termsText: { fontSize: 11, color: "rgba(255,255,255,0.2)", textAlign: "center", flex: 1 },

  // Section divider
  sectionDivRow:   { flexDirection: "row", alignItems: "center", gap: 10, marginVertical: 16 },
  sectionDivLine:  { flex: 1, height: 1, backgroundColor: "rgba(255,255,255,0.07)" },
  sectionDivLabel: { fontSize: 11, color: "rgba(255,255,255,0.2)", fontWeight: "600" },

  // Step dots (kept for future step UI)
  dotsRow:   { flexDirection: "row", alignItems: "center", justifyContent: "center", marginBottom: 24, position: "relative" },
  dot:       { width: 18, height: 18, borderRadius: 9, backgroundColor: "rgba(255,255,255,0.08)", borderWidth: 1, borderColor: "rgba(255,255,255,0.12)", alignItems: "center", justifyContent: "center", zIndex: 1 },
  dotActive: { backgroundColor: "#6C63FF", borderColor: "#8B5CF6" },
  dotDone:   { backgroundColor: "#059669", borderColor: "#10B981" },
  dotTrack:  { position: "absolute", height: 1, backgroundColor: "rgba(255,255,255,0.08)" },
